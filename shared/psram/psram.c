...
static const struct psram_type_t psram_chip_types[] = {
    /* ID             Name        Max Clk (Mhz)   tCEM (fs)    tCPM (fs)   tCPM
       (cs) */
...
static inline void csr_enable_direct_mode(uint8_t csr_clkdiv) {
  ...
  qmi_hw->direct_csr =
      csr_clkdiv << QMI_DIRECT_CSR_CLKDIV_LSB | QMI_DIRECT_CSR_EN_BITS;
  csr_busy_wait();
}

static inline void csr_disable_direct_mode() {
  /* Disable direct mode */
  hw_clear_bits(&qmi_hw->direct_csr,
...
static void __no_inline_not_in_flash_func(psram_read_id)(uint8_t csr_clkdiv,
                                                         uint8_t *buffer) {
  ...
  csr_send_command(QMI_DIRECT_TX_OE_BITS |
                       QMI_DIRECT_TX_IWIDTH_VALUE_Q
                           << QMI_DIRECT_TX_IWIDTH_LSB |
                       CMD_EXIT_QPI_MODE,
                   NULL);
...
static void __no_inline_not_in_flash_func(psram_qmi_setup)(uint8_t clkdiv,
                                                           uint8_t csr_clkdiv,
                                                           uint8_t max_select,
                                                           uint8_t min_deselect,
                                                           uint8_t rxdelay) {
  uint32_t saved_ints = save_and_disable_interrupts();
  ...
  qmi_hw->m[1].rcmd =
      (0 << QMI_M1_RCMD_SUFFIX_LSB | CMD_QUAD_READ << QMI_M1_RCMD_PREFIX_LSB);
  ...
  qmi_hw->m[1].wcmd =
      (0 << QMI_M1_WCMD_SUFFIX_LSB | CMD_QUAD_WRITE << QMI_M1_WCMD_PREFIX_LSB);
...
static int psram_init(int pin, bool clear_memory) {
  ...
  /* Calculate clock divider to get as close as possible to the max supported
   * clock speed */
  ...
  uint8_t min_deselect =
      (chip->min_deselect_fs + clock_period_fs - 1) / clock_period_fs;
...
void psram_setup(int pin) {
  int res = psram_init(pin, true);
  if (res == -3) {
...
size_t psram_size() { return psram_sz; }

const psram_id_t *psram_get_id() { return (const psram_id_t *)&psram_id; }
