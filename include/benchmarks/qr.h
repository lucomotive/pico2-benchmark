#pragma once
#include "../Benchmark.h"

#include "pico/rand.h"
#include "pico/stdlib.h"
#include "pico/time.h"

#include <iostream>

#include <nlohmann/json.hpp>

#include <Eigen/Dense>

template <typename P> class QR : public Benchmark {
private:
  uint32_t min_dim;
  uint32_t max_dim;

public:
  QR(uint32_t min_dim, uint32_t max_dim) : max_dim(max_dim), min_dim(min_dim) {}
  void run(bool debug) override;

  void sof() override;
};
template <typename P> void QR<P>::sof() {
  printf("SOF\n");
  printf("X,Y,time_us\n");
}
template <typename P> void QR<P>::run(bool debug) {
#define Dyn Eigen::Dynamic
#define GenericMatrix Eigen::Matrix<P, Dyn, Dyn>

  uint32_t x = debug ? 5 : Benchmark::random_range_32(min_dim, max_dim);
  uint32_t y = debug ? 5 : Benchmark::random_range_32(min_dim, max_dim);

  GenericMatrix A(x, y);
  // random values
  A.setRandom();

  // start clock
  absolute_time_t startTime = get_absolute_time();

  Eigen::HouseholderQR<GenericMatrix> qr(A);
  GenericMatrix thinQ(GenericMatrix::Identity(x, y));

  GenericMatrix Q(qr.householderQ() * thinQ);
  GenericMatrix R(Q.transpose() * A);

  // stop clock
  absolute_time_t stopTime = get_absolute_time();

  // prevent optimization (hopefully)
  volatile P sink = R(0, 0);
  (void)sink;

  uint64_t time_us = absolute_time_diff_us(startTime, stopTime);
  if (!debug) {
    printf("%lu,%lu,%llu\n", x, y, time_us);
  }

  // debug results
  if (debug) {
    std::cout << "–––––––––––––––––––––Testing algorithm––––––––––––––––––––––"
              << std::endl;

    std::cout << "A = " << std::endl << A << std::endl;
    std::cout << "Q = " << std::endl << Q << std::endl;
    std::cout << "R = " << std::endl << R << std::endl;

    // reverse engineer this
    GenericMatrix approxA(Q * R);
    double error = (A - approxA).squaredNorm();
    std::cout << "A ≈ " << std::endl << approxA << std::endl;
    std::cout << "Error = " << error << std::endl;

    std::cout << "Time µs = " << time_us << std::endl;

    std::cout << "––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––"
              << std::endl;
  }
}
