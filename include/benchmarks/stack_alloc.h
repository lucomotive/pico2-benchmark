...
template <typename P> class StackAlloc : public Benchmark {
private:
  uint32_t min_dim;
  uint32_t max_dim;

public:
  StackAlloc(uint32_t min_dim, uint32_t max_dim)
      : max_dim(max_dim), min_dim(min_dim) {}
  void run(bool debug) override;

  void sof() override;
};
template <typename P> void StackAlloc<P>::sof() {
  printf("SOF\n");
  printf("X,Y,time_us\n");
}
template <typename P> void StackAlloc<P>::run(bool debug) {
#define Dyn Eigen::Dynamic
#define GenericStackMatrix Eigen::Matrix<P, Dyn, Dyn, 0, 100, 100>
