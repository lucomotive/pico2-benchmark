...
template <typename P> class Copy : public Benchmark {
private:
  uint32_t min_dim;
  uint32_t max_dim;

public:
  Copy(uint32_t min_dim, uint32_t max_dim)
      : max_dim(max_dim), min_dim(min_dim) {}
  void run(bool debug) override;

  void sof() override;
};
template <typename P> void Copy<P>::sof() {
  printf("SOF\n");
  printf("X,Y,time_us\n");
}
template <typename P> void Copy<P>::run(bool debug) {
