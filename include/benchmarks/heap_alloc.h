...
template <typename P> class HeapAlloc : public Benchmark {
private:
  uint32_t min_dim;
  uint32_t max_dim;

public:
  HeapAlloc(uint32_t min_dim, uint32_t max_dim)
      : max_dim(max_dim), min_dim(min_dim) {}
  void run(bool debug) override;

  void sof() override;
};
template <typename P> void HeapAlloc<P>::sof() {
  printf("SOF\n");
  printf("X,Y,time_us\n");
}
template <typename P> void HeapAlloc<P>::run(bool debug) {
